...
def select_frame_parts(developed_parts):
    ...
    while True:
        ...
        # Get next joint part
        if unused_joints:
            bi = [i for i in range(len(used_bodys))
...
def select_neuron_parts(developed_parts, frame_parts):
    ...
    unused_neurons = list(range(len(neuron_list)))
    used_neurons = list()
    ...
    for i in itertools.cycle(list(range(len(neuron_mounts)))):
...
def select_sensor_parts(developed_parts, frame_parts):
    ...
    unused_sensors = list(range(len(sensor_list)))
    used_sensors = list()
    body_list = [i for i in developed_parts
                 if (i.__class__ == BodyPart and
                     i.characteristics.joint_mount_num > 0)]
    ...
    for i in itertools.cycle(list(range(len(sensor_mounts)))):
...
def make_wire_part(part, case, generators):
    ...
    if case == 'S->J':
        return Wire(wire=part[0],
                    ...
                    from_index=next(generators[3]),
                    to_type=JointPart,
                    to_index=next(generators[0]))
    elif case == 'S->N':
        return Wire(wire=part[0],
                    from_type=SensorPart,
                    from_index=next(generators[3]),
                    to_type=NeuronPart,
                    to_index=next(generators[1]))
    elif case == 'N->J':
        return Wire(wire=part[0],
                    from_type=NeuronPart,
                    from_index=next(generators[2]),
                    to_type=JointPart,
                    to_index=next(generators[0]))
    elif case == 'N->N':
        return Wire(wire=part[0],
...
def select_wire_parts(developed_parts, frame_parts,
    ...
    joint_in_generator = peekable(itertools.cycle(list(range(len(joint_ins)))))
    neuron_in_generator = peekable(itertools.cycle(list(range(len(neuron_ins)))))
    neuron_out_generator = peekable(itertools.cycle(list(range(len(neuron_outs)))))
    sensor_out_generator = peekable(itertools.cycle(list(range(len(sensor_outs)))))
